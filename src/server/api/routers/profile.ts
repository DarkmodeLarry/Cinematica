...
export const profileRouter = createTRPCRouter({
  ...
  getOthers: protectedProcedure.input(z.string()).query(async ({ ctx, input }) => {
    const profiles = await ctx.prisma.profile.findMany({
      where: { userId: ctx.session.user.id },
      include: {
        icon: true
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const profileName = profiles.find((profile) => profile.name === input.name)
      if (profileName) {
        throw new TRPCError({
          code: 'FORBIDDEN',
      ...
      const profileIcon = profiles.find((profile) => profile.iconId === input.iconId)
      if (profileIcon) {
        throw new TRPCError({
          code: 'FORBIDDEN',
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (profileGameHandle && profileGameHandle.gameHandle && profileGameHandle.id !== input.id) {
        throw new TRPCError({
          code: 'FORBIDDEN',
