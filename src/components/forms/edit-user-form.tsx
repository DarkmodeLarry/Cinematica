...
const EditUserForm = ({ user }: EditUserFormProps) => {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    ...
    await updateUserMutation.mutateAsync({
      id: user.id,
      email: data.email,
      phoneNumber: data.phoneNumber && data.phoneNumber.length > 0 ? data.phoneNumber : null
    })
  }
    ...
    <div className='flex flex-col gap-5'>
      <div className='space-y-2.5'>
        ...
        <div className='flex items-center gap-2.5'>
          <Icons.edit2 className='w-6 h-6 text-neutral-500' />
          <p className='text-sm font-medium text-neutral-500'>Update your account details.</p>
        </div>
      </div>
