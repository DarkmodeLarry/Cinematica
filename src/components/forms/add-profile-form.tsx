...
const AddProfileForm = ({ profiles, profileIcon }: AddProfileFormProps) => {
  ...
  return (
    <AnimatePresence>
      {iconPicker ? (
        <ProfilePicker icon={icon} setIconPicker={setIconPicker} setIcon={setIcon} />
      ) : (
        <motion.div
          className='container flex flex-col justify-center w-full max-w-2xl min-h-screen gap-3'
        ...
        >
          ...
          >
            <div className='flex flex-col w-full gap-6 sm:flex-row sm:items-center'>
              <Button
                ...
                disabled={profiles.length >= 5 || createProfileMutation.isLoading}
              >
                <Image
                  src={icon.href}
                  alt={icon.title}
              ...
              <fieldset className='grid items-start flex-1 w-full gap-2'>
