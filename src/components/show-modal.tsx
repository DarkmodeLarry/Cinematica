...
const ShowModal = ({ open, setOpen }: ShowModalProps) => {
  ...
  React.useEffect(() => {
    const getShow = async () => {
      ...
      try {
        ...
        if (data?.videos) {
          const trailerIndex = data.videos.results.findIndex((item) => item.type === 'Trailer')
  ...
  return (
    <Dialog
      ...
      open={open}
    >
      <DialogContent className='w-full p-0 overflow-hidden text-left align-middle rounded-md shadow-xl bg-zinc-900 dark:bg-zinc-900 sm:max-w-3xl'>
        <div className='relative aspect-video'>
          ...
          />
          <div className='absolute z-20 flex items-center justify-between w-full gap-2 px-10 bottom-6'>
            <div className='flex items-center gap-2.5'>
              ...
              >
                {isPlaying ? (
                  <>
                    <Icons.pause className='mr-1.5 h-6 w-6 fill-current' aria-hidden='true' />
                    Pause
                  </>
                ) : (
                  <>
                    <Icons.play className='mr-1.5 h-6 w-6 fill-current' aria-hidden='true' />
                    Play
                  </>
                )}
              </Button>
              <DynamicTooltip text={isAdded ? 'Remove from My List' : 'Add to My List'}>
                <Button
                  ...
                  onClick={() => {
                    ...
                    isAdded || myShowsQuery?.data?.find((item) => item.id === modalStore.show?.id)
                      ...
                      : addShowMutation.mutate({
                          ...
                          media_type: modalStore.show.media_type === 'tv' ? 'tv' : 'movie',
                ...
                >
                  {isAdded ||
                  myShowsQuery?.data?.find((item) => item.id === modalStore.show?.id) ? (
                    <Icons.check className='w-5 h-5' aria-hidden='true' />
                  ) : (
                    <Icons.add className='w-5 h-5' aria-hidden='true' />
                  )}
                </Button>
              </DynamicTooltip>
            </div>
            ...
            >
              {isMuted ? (
                <Icons.volumneMute className='w-6 h-6' aria-hidden='true' />
              ) : (
                <Icons.volumne className='w-6 h-6' aria-hidden='true' />
              )}
            </Button>
          </div>
        </div>
        <div className='grid gap-2.5 px-10 pb-10'>
          ...
          <div className='flex items-center space-x-2 text-sm sm:text-base'>
            ...
            {modalStore.show?.original_language && (
              <span className='grid h-4 text-xs font-bold w-7 place-items-center text-neutral-400 ring-1 ring-neutral-400'>
                {modalStore.show.original_language.toUpperCase()}
              </span>
            )}
          </div>
          <DialogDescription className='text-xs line-clamp-3 text-slate-50 dark:text-slate-50 sm:text-sm'>
